...
namespace TheSlum.GameEngine
{
    ...
    public class Engine
    {
        ...
        protected virtual void CreateCharacter(string[] inputParams)
        {
            ...
            string characherId = inputParams[2];
            int charachterX = int.Parse(inputParams[3]);
            int charachterY = int.Parse(inputParams[4]);
            Team charachterTeam = (Team)Enum.Parse(typeof(Team), inputParams[5], true);

            switch (inputParams[1])
            {
                case "warrior":
                    currentCharacter = new Warrior(characherId, charachterX, charachterY, 100, 100, 150, charachterTeam, 5);
                    this.characterList.Add(currentCharacter);
                    break;
                case "mage":
                    currentCharacter = new Mage(characherId, charachterX, charachterY, 90, 70, 120, charachterTeam, 3);
                    this.characterList.Add(currentCharacter);
                    break;
                case "healer":
                    currentCharacter = new Healer(characherId, charachterX, charachterY, 110, 80, 160, charachterTeam, 3);
                    this.characterList.Add(currentCharacter);
                    break;
                default:
                    break;
        ...
        {
            Item itemToAdd;
            Character charachterToAddnewItem = GetCharacterById(inputParams[1]);

            switch (inputParams[2])
            {
                case "axe":
                    itemToAdd = new Axe(inputParams[3]);
                    charachterToAddnewItem.AddToInventory(itemToAdd);
                    break;
                case "shield":
